...
switch(backpackState){
	...
	#region BackpackState.BOTTOM
	//-------- draw self ---------------
	draw_sprite_ext(SPR_BOTTOM_INVENTORY,0,BOTTOM_INVENTORY_X,BOTTOM_INVENTORY_Y,
		INVENTORY_SCALE,INVENTORY_SCALE,0,c_white,1
	);
	...
	var itemNum,itemType;
	var item_x_of_sprite,item_y_of_sprite;
	j=BACKPACK_SLOTS_HEIGHT-1;//only last row
	for(i=0;i<BACKPACK_SLOTS_WIDTH;i++){
		
		
		
			//get draw position
			slot_x=BOTTOM_SLOTS_X+i*(SLOTS_NORMAL_SPACE+UNIT)*INVENTORY_SCALE;
			slot_y=BOTTOM_SLOTS_Y;
			...
			var handingSlotIndex=m_handingSlotSubIndex+BACKPACK_SLOTS_WIDTH*(BACKPACK_SLOTS_HEIGHT-1);
			if(slot_index==handingSlotIndex){
				gpu_set_blendmode(bm_add);
				draw_sprite_ext(spr_inventorySlot,0,slot_x,slot_y,INVENTORY_SCALE,INVENTORY_SCALE,0,c_white,0.5);
				gpu_set_blendmode(bm_normal);
			}
